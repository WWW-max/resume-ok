...
import { Download, Eye, PencilLine, LayoutGrid, Sparkles, Files, Settings, Monitor, ChevronDown, Lightbulb } from "lucide-react";
export default function Home() {
  ...
  return <div className={`workspace ${previewOnly ? "preview-only" : ""}`}>
    ...
    <div className="workspace-body"><nav className="side-nav" aria-label="主导航">{[{id:"editor",label:"编辑简历",icon:PencilLine},{id:"templates",label:"模板库",icon:LayoutGrid},{id:"assistant",label:"简历优化",icon:Sparkles},{id:"resumes",label:"我的简历",icon:Files},{id:"settings",label:"设置",icon:Settings}].map(({id,label,icon:Icon}) => <button key={id} className={view===id ? "active" : ""} onClick={() => {setView(id);setPreviewOnly(false);}}><Icon size={21} /><span>{label}</span></button>)}</nav>
    <aside className="editing-column">{view === "editor" ? <EditorPanel data={data} onChange={setData} onAssistant={() => setView("assistant")} /> : <div className="panel-heading"><h1>{({templates:"模板库",assistant:"简历助手",resumes:"我的简历",settings:"设置"} as Record<string,string>)[view]}</h1><p>当前简历：{data.name} · {data.title}</p><button className="back-button" onClick={() => setView("editor")}>返回编辑简历</button></div>}</aside>
    <main className="preview-column" aria-label="简历预览"><div className="preview-toolbar"><div className="toolbar-group"><Monitor size={18} /><span className="toolbar-pill">自适应</span></div><div className="toolbar-group"><button className="blue-button" onClick={() => setView("templates")}><LayoutGrid size={15} />模板库</button><button className="blue-button" onClick={() => setView("assistant")}><Sparkles size={15} />简历检查</button><span className="toolbar-pill">A4 (210 × 297mm)</span></div></div><PreviewCanvas data={data} previewRef={previewRef} /></main>
    </div>
  </div>;
}
