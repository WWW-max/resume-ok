...
export const ResumePreview = forwardRef<HTMLDivElement, { data: ResumeData }>(({ data }, ref) => {
  ...
  function content(key: SectionKey) {
    switch (key) {
      ...
      case "skills": return data.skills.length ? <div className="skill-tags">{data.skills.flatMap(s => s.items.split(/\s*[/、,，]\s*/).filter(Boolean)).map((s,i) => <span key={i}>{s}</span>)}</div> : null;
      case "workExperiences": return data.workExperiences.length ? data.workExperiences.map(item => <article key={item.id}><div className="resume-row"><h3>{item.company} <span>{item.position}</span></h3><time>{item.startDate} — {item.current ? "至今" : item.endDate}</time></div><Description text={item.description} /></article>) : null;
      case "projects": return data.projects.length ? data.projects.map(item => <article key={item.id}><div className="resume-row"><h3>{item.name}</h3><time>{item.startDate} — {item.endDate}</time></div><p className="resume-role">{item.role}</p>{item.link && <p className="resume-link">{item.link}</p>}<Description text={item.description} /></article>) : null;
  ...
  return <div ref={ref} className={`resume-paper template-${appearance.template} spacing-${appearance.spacing}`} style={{"--resume-accent":accentColors[appearance.accent],"--resume-font-size":`${appearance.fontSize}px`} as CSSProperties}><Clover className="paper-watermark" aria-hidden="true" />{appearance.showBrand && <Brand small />}
    <header className="resume-header"><div><h1>{data.name || "您的姓名"}</h1><p className="resume-job">{data.title || "求职意向"}</p><div className="resume-contact">
      {[[Phone, data.phone], [Mail, data.email], [MapPin, data.location], [LinkIcon, data.github], [LinkIcon, data.website]].map(([Icon, text], i) => {
        ...
        return typeof text === "string" && text ? <span key={i}><ContactIcon size={14} />{text}</span> : null;
      })}</div></div>{data.avatar &&
