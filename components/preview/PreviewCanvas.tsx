...
export function PreviewCanvas({ data, previewRef }: { data: ResumeData; previewRef: RefObject<HTMLDivElement | null> }) {
  ...
  useEffect(() => {
    ...
    const observer = new ResizeObserver(() => setSize({ scale: Math.min(1, Math.max(0.2, (outer.clientWidth - 64) / 794)), height: paper.offsetHeight }));
