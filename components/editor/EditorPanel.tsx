...
import { User, Briefcase, GraduationCap, FolderOpen, Zap, FileText } from "lucide-react";
        ...
        <TabsList className="w-full grid grid-cols-6 bg-white/5 border border-white/10 rounded-xl p-1 h-auto gap-0.5">
          {tabs.map(({ id, label, icon: Icon }) => (
            <TabsTrigger
              key={id}
              value={id}
              className="flex flex-col items-center gap-0.5 py-2 px-1 rounded-lg text-white/40 data-[state=active]:text-white data-[state=active]:bg-white/10 transition-all duration-200 hover:text-white/70 text-[10px] font-medium"
            >
